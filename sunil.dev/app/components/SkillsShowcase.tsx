...
export function SkillsShowcase({ groups, extras }: Props) {
  ...
  return (
    <div className="relative space-y-14">
      <div
        className="pointer-events-none absolute -right-24 top-0 h-72 w-72 rounded-full bg-gradient-to-br from-violet-400/15 via-transparent to-emerald-400/10 blur-3xl dark:from-violet-500/10 dark:to-emerald-500/10"
        aria-hidden
      />
      ...
      <section className="relative space-y-6">
          ...
          {groups.map((g) => {
            ...
            return (
              <button
                ...
                className={`rounded-full border px-4 py-2 text-sm lowercase transition-all duration-300 ${
                  isOn
        ...
        {activeGroup ? (
          <div key={activeGroup.id} className="skills-group-enter space-y-6">
              ...
              {activeGroup.skills.map((skill, i) => (
                <li key={skill.name}>
                  ...
                  <div className="mt-2 h-2 overflow-hidden rounded-full bg-zinc-200/90 dark:bg-zinc-800">
                    <div
                      className="skill-meter-inner h-full rounded-full bg-gradient-to-r from-zinc-700 via-zinc-800 to-zinc-900 dark:from-zinc-300 dark:via-zinc-200 dark:to-zinc-100"
                      style={
                        {
                          "--fill": `${skill.level}%`,
                          "--delay": `${120 + i * 70}ms`,
      ...
      <section className="relative space-y-4">
        ...
        <p className="max-w-prose text-sm text-zinc-600 dark:text-zinc-400">
          Smaller surface area, but I&apos;ve shipped or debugged with these
        ...
        <div className="flex flex-wrap gap-2">
          {extras.map((label, i) => (
            <span
              key={label}
              className="skill-pill inline-flex items-center rounded-full border border-zinc-200/90 bg-white/70 px-3 py-1.5 text-xs lowercase text-zinc-700 shadow-sm backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-zinc-300 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900/60 dark:text-zinc-300 dark:hover:border-zinc-500"
